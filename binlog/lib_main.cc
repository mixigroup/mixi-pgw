        ...
        )
        {
            ...
            for (size_t i = 0; i < td->size(); i++) {
                ...
                case MYSQL_TYPE_LONGLONG:
                    ...
                    length = 8;
                    nval = *(uint64_t*)value;
                    pnotify_->OnNotify(type_, i, Notify::TYPE_NUMERIC, (void*)value, 8, data_);
                    break;
                ...
                case MYSQL_TYPE_VAR_STRING:
                    ...
                    // string value
                    length ++;
                    pnotify_->OnNotify(type_, i, Notify::TYPE_TEXT, txt, strlen(txt), data_);
                    break;
                default:
            ...
            if (type_ == UPDATE_ROWS_EVENT) {
                value += (bitmap_bits_set(&m_cols_ai) + 7) / 8;
                for (size_t i = 0; i < td->size(); i++) {
                    if (bitmap_is_set(&m_cols_ai, i) == 0) { continue; }
                    // 
                    switch(td->type(i)) {
                        case MYSQL_TYPE_LONGLONG:
                            // numeric value
