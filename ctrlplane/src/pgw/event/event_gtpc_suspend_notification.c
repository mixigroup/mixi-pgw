...
/*! @addtogroup mixi_pgw_ctrl_plane
    @file       event_gtpc_suspend_notification.c
    @brief      gtpc : suspend notification receive event
*******************************************************************************
*******************************************************************************
 ...
 */
RETCD event_gtpc_suspend_notification_req(packet_ptr pckt, void* ext){
    ...
    PGW_LOG(PGW_LOG_LEVEL_DBG, "event_gtpc_suspend_notification_req..(%p : %p : %u/%p)\n", pnode, (void*)pthread_self(), pnode->index, pckt);
    bzero(&parse_state, sizeof(parse_state));

    // collect parameters necessary for Suspend Notification Response from Request packet
    ...
    // update status by Suspend Notification Request
    snprintf(sql, sizeof(sql)-1, SUSPEND_NOTIFY_UPD_SQL, parse_state.gtph_teid);
    //
    pthread_mutex_lock(&__mysql_mutex);
...
/**
  Susspend Notification Response Accept\n
 *******************************************************************************
 \n
 *******************************************************************************
 @parma[in]   rpckt   1packet 
 @param[in]   pnode  node object
 @param[in]   cs     Susspend Notification processing container
 @return int  0==OK,0!=error
 */
RETCD _accept(gtp_packet_ptr rpckt, node_ptr pnode, struct gtpc_parse_state* cs){
    ...
    //
    while(1){
        if (gtpc_alloc_packet(&lpckt, GTPC_SUSPEND_ACK) != OK){ break; }
    ...
    if (pgw_enq(pnode, EGRESS, cpckt) != OK){
        pgw_panic("failed. enq(_accept suspend ack response).");
    }
    return(OK);
}
/**
  Suspend Notification Response Denied \n
 *******************************************************************************
 \n
 *******************************************************************************
 @parma[in]   rpckt   1packet 
 @param[in]   pnode  node object
 @param[in]   cs     Suspend Notification Processing container
 @return int  0==OK,0!=error
 */
RETCD _denied(gtp_packet_ptr rpckt, node_ptr pnode,struct gtpc_parse_state* cs){
    ...
    // enqueue packet into reply ring
    if (pgw_enq(pnode, EGRESS, cpckt) != OK){
        pgw_panic("failed. enq(_denied  suspend notification response).");
    }
    return(OK);
}
