...
/*! @addtogroup mixi_pgw_ctrl_plane
    @file       event_gtpc_delete_session.c
    @brief      gtpc : delete session receive event
*******************************************************************************
*******************************************************************************
 ...
 */
RETCD event_gtpc_delete_session_req(packet_ptr pckt, void* ext){
    ...
    PGW_LOG(PGW_LOG_LEVEL_DBG, "event_gtpc_delete_session_req..(%p : %p : %u/%p)\n", pnode, (void*)pthread_self(), pnode->index, pckt);
    bzero(&parse_state, sizeof(parse_state));

    // collect parameters necessary for Delete Session Response from request packet
    ...
    // update status by Delete Session Request parameter
...
/**
  Delete Session Response Accept\n
 *******************************************************************************
 \n
 *******************************************************************************
...
RETCD _accept(gtp_packet_ptr rpckt, node_ptr pnode, struct gtpc_parse_state* cs){
    ...
    if (pgw_enq(pnode, EGRESS, cpckt) != OK){
        pgw_panic("failed. enq(_accept delete session response).");
    }
    return(OK);
}
/**
  Delete Session Response Denied \n
 *******************************************************************************
 \n
 *******************************************************************************
 @parma[in]   rpckt   1packet 
...
RETCD _denied(gtp_packet_ptr rpckt, node_ptr pnode,struct gtpc_parse_state* cs){
    ...
    if (pgw_enq(pnode, EGRESS, cpckt) != OK){
        pgw_panic("failed. enq(_denied  delete session response).");
    }
    return(OK);
}
