...
static void *packet_enqueuer(void *param){
    while (qloop != 1) {
        ...
        set_packet_temp(pkt,PTEMP_ICMP_IPV4_GRE_IPOPTION_IPV4);
...
static void *packet_dequeuer(void *param) {
    ...
    clock_gettime(CLOCK_THREAD_CPUTIME_ID, &deq_ts);
}

TEST(PgwEgressDistributorWorker, Init){
    std::string uri = "mysql://root:develop@127.0.0.1:3306";
    __target = new CorePgwEgressDistributorWorker(0,uri.c_str(),12005,0,PGW_TUNNEL_IP,PGW_TUNNEL_IP,0,0);
...
TEST(PgwEgressDistributorWorker, ThroughPut){
    ...
    printf("DEQ CPU TIME : %4ld%03ldms \n", deq_ts.tv_sec, deq_ts.tv_nsec / 1000000);
    auto pkt = rte_pktmbuf_alloc(__pktmbuf_pool_);
    set_packet_temp(pkt,PTEMP_ICMP_IPV4_GRE_IPOPTION_IPV4);
