...
#define         PKTSIZE     (234)
#define         TEID        (0x00000000)
#define         DSTADDR     (0x99887766)
#define         SRCADDR     (0x11223344)
...
TEST(PgwIngressDistributorWorker, Init){
    ...
    __rxdist_0->SetRingAddr(ORDER::TO, __rings_[4]);     // not use Ingress-side
    __rxdist_0->SetRingAddr(ORDER::TO, __rings_[1]);
    __pgw_e->SetRingAddr(ORDER::FROM, __rings_[1]);
    __pgw_e->SetRingAddr(ORDER::TO, __rings_[2]);
    ...
    __pgw_e->SetN(MIXIPGW::KEY::BURST_FROM, 1);
    __pgw_e->SetN(MIXIPGW::KEY::BURST_TO, 1);
    __tx_0->SetN(MIXIPGW::KEY::BURST_FROM, 1);
    __tx_0->SetN(MIXIPGW::KEY::BURST_TO, 1);
    __pgw_e->BeforeCycle(NULL);
}
...
TEST(PgwIngressDistributorWorker,ALLTest){
    ...
    set_packet_gtpu(pkt, 0xde, 0xad, htonl(SRCADDR), htonl(DSTADDR), 50999, 50999, htonl(TEID), PKTSIZE);
    set_packet_gtpu(compare, 0xde, 0xad, htonl(SRCADDR), htonl(DSTADDR), 50999, 50999, htonl(TEID), PKTSIZE);
    ...
    pthread_create(&th, NULL, [](void* arg)->void*{
        uint64_t count = 0;
        for(;;){
            __pgw_e->VirtualCycle(NULL, &count);
        }
        return((void*)NULL);
    }, NULL);
    sleep(2);
    ...
    if (nburst == 1){
        ...
        // equal length (gtpu + udp = gre + ipoption)
