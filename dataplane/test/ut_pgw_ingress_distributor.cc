...
static MIXIPGW::CorePgwIngressDistributorWorker* __target = NULL;
...
TEST(DistributorPgwIngress, Init){
    EXPECT_EQ(truncate_tunnel(), 0);

    std::string uri = "mysql://root:develop@127.0.0.1:3306";

    __target = new MIXIPGW::CorePgwIngressDistributorWorker(0,uri.c_str(),13001,1,0x11111111,0x22222222,0x02,0x03);
...
TEST(DistributorPgwIngress, TapTx){
    ...
    set_packet_gtpu(pkt, SRCMAC, DSTMAC, htonl(SRCADDR), DSTADDR, SRCPORT, DSTPORT, htonl(TEID), PKTSIZE);
    //
    ...
    EXPECT_EQ(nburst,1);
    if (nburst != 1){ return; }
    // not found tunnel id -> error notify trigger to tx
...
//  pthread_join(th, NULL);
    ...
    auto nop= rte_pktmbuf_mtod_offset(input[0], char*, sizeof(ether_hdr) + sizeof(ipv4_hdr));
    auto gre= rte_pktmbuf_mtod_offset(input[0], char*, sizeof(ether_hdr) + sizeof(ipv4_hdr) + sizeof(nops));
    EXPECT_EQ(memcmp(nop, nops, sizeof(nops)), 0);
    EXPECT_EQ(memcmp(gre, greo, sizeof(greo)), 0);
    fprintf(stderr, "...\n");
}

TEST(DistributorPgwIngress, UnInit){
    EXPECT_EQ(__target!=NULL,true);
    if (__target){
        delete __target;
    }
