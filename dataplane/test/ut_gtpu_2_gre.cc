...
typedef struct utparam{
    MIXIPGW::CoreGtpu2GreWorker*  target_;
...
static void ut_ingress(void) {
    ...
    auto gtp= (struct MIXIPGW::gtpu_hdr*)(udp+1);
    ...
    //
    gtp->u.flags = 0;
    gtp->u.v1_flags.version = 1;
    gtp->u.v1_flags.proto   = 1;
    gtp->type    = 0xff;
    gtp->tid     = 0xdeadbeaf;
    gtp->length  = rte_cpu_to_be_16(TEST_BUFFER_SZ - sizeof(*eh) - sizeof(*udp));
    //
...
static void ut_exchanged(struct rte_mbuf* pkt){
    // packet size is not changed.
    assert(pkt->pkt_len==TEST_BUFFER_SZ);
    assert(pkt->data_len==TEST_BUFFER_SZ);

    // out-side ip address
    auto ip = rte_pktmbuf_mtod_offset(pkt, struct ipv4_hdr*, 14);
    assert(ip->src_addr==(uint32_t)__counter_r);
    assert(ip->dst_addr==(uint32_t)(TUNNEL_FIXED_SRCIP));
    assert(ip->next_proto_id==IPPROTO_GRE);
    assert(ip->version_ihl==(IP_VERSION | 0x08));


    // sequence number in payload
    auto payload = rte_pktmbuf_mtod_offset(pkt, uint32_t*, 50);
...
static void ut_egress(void) {
    ...
    if (ret == 0){
        if (RTE_ETH_IS_IPV4_HDR(pkt->packet_type)) {
            ...
            rte_exit(EXIT_FAILURE, "not implemented not ipv4...\n");
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    __ingress.target_ = new MIXIPGW::CoreGtpu2GreWorker(0,TUNNEL_FIXED_SRCIP);
