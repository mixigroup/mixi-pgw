...
#define TUNNEL_FIXED_SRCIP  (0x01020304)
...
static void ut_exchanged(struct rte_mbuf* pkt){
    // packet size is reduced by ip(32) + gre
    assert(pkt->pkt_len==(TEST_BUFFER_M_SZ-36));
    assert(pkt->data_len==(TEST_BUFFER_M_SZ-36));

    // ipv6 address out-side , inner-side ipv6 header before decapsulate
    auto eh = rte_pktmbuf_mtod(pkt, struct ether_hdr*);
    auto ip = rte_pktmbuf_mtod_offset(pkt, struct ipv6_hdr*, sizeof(ether_hdr));
    for(auto n = 0;n < 16;n ++){
        assert(ip->src_addr[n]==n);
        assert(ip->dst_addr[15-n]==n);
    }
    assert(ip->proto==IPPROTO_UDP);
    assert((ntohl(ip->vtc_flow)&0xf0000000)==0x60000000);

    // ethertype == ipv6(86dd)
    assert(eh->ether_type == rte_cpu_to_be_16(ETHER_TYPE_IPv6));
...
static void ut_egress(void) {
    ...
    if (ret == 0){
        if (RTE_ETH_IS_IPV4_HDR(pkt->packet_type)) {
            rte_exit(EXIT_FAILURE, "not implemented ipv4...(ut_egress)\n");
        }else if (RTE_ETH_IS_IPV6_HDR(pkt->packet_type)) {
            __counter_r++;
            ut_exchanged(pkt);
        }else{
            rte_exit(EXIT_FAILURE, "not implemented not ipv4 and ipv6...(ut_egress)\n");
        }
    }
}
