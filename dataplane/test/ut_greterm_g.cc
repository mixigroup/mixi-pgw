...
static MIXIPGW::CoreGretermWorker* __target = NULL;

TEST(GretermWorker, Init){
    __target = new MIXIPGW::CoreGretermWorker(0,TUNNEL_FIXED_SRCIP);
...
TEST(GretermWorker,IPv4Test){
    const uint8_t   PKTSIZE  = 234;
...
TEST(GretermWorker,IPv6Test){
    ...
    set_packet_gtpu_ipv6(pkt, 0xde, 0xad, htonl(SRCADDR), htonl(DSTADDR), 2152, 2152, htonl(TEID), PKTSIZE);
    set_packet_gtpu_ipv6(compare, 0xde, 0xad, htonl(SRCADDR), htonl(DSTADDR), 2152, 2152, htonl(TEID), PKTSIZE);
    ...
    if (nburst == 1){
        ...
        EXPECT_EQ(*(uint16_t *)(prev+sizeof(struct ether_hdr)-2),rte_cpu_to_be_16(ETHER_TYPE_IPv4)); 
        // ingress packet Ethertype == 0x0800

        EXPECT_EQ(*(uint16_t *)(current+sizeof(struct ether_hdr)-2),rte_cpu_to_be_16(ETHER_TYPE_IPv6));
        // egress packet Ethertype == 0x86dd
    }
}

TEST(GretermWorker,vlan_icmpTest){
    uint64_t count = 0;
    Buf input[DEFAULT_MBUF_ARRAY_SIZE];
...
TEST(GretermWorker,ospf_ipv4Test){
    uint64_t count = 0;
    Buf input[DEFAULT_MBUF_ARRAY_SIZE];
...
TEST(GretermWorker,bfd_udp_ipv4Test){
    uint64_t count = 0;
    Buf input[DEFAULT_MBUF_ARRAY_SIZE];
...
TEST(GretermWorker,bgp_tcp_ipv4Test){
    uint64_t count = 0;
    Buf input[DEFAULT_MBUF_ARRAY_SIZE];
