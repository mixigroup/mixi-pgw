...
#define TEST_PACKET PTEMP_TCP_IPV4
...
static void *packet_enqueuer(void *param){
    while (qloop != 1) {
        ...
        set_packet_ipv4_gre(pkt,SRCMAC,DSTMAC,SRCADDR,DSTADDR);
...
TEST(GretermWorker, Init){
    __target = new MIXIPGW::CoreGretermWorker(1,0);
